  ...
  return (
    <div className="bg-white shadow-lg overflow-hidden">
      {pendingPayments.length > 0 ? (
          ...
          <tbody>
            {pendingPayments.map((payment) => (
              <tr key={payment._id} className="border-b hover:bg-gray-50">
                ...
                <td className="p-3">
                  {FEE_TYPE_OPTIONS.find((fee) => fee.value === payment.feeType)?.label || "Tuition Fees"}
                </td>
                <td className="p-3 font-medium">₹{payment.amount}</td>
                <td className="p-3">{payment.paymentMethod}</td>
                <td className="p-3 text-red-600 font-semibold">{payment.transactionId}</td>
                <td className="p-3">
                  {payment.paidAt
        ...
        <div className="bg-gray-400 p-4 rounded-xl flex items-center justify-center text-rose-700 font-bold">
          No One Request
        </div>
      )}
    </div>
